...
<script>
	...
	export default{
		...
		//01_5.通过计算属性给nickName赋值
		computed:{
			nickName(){
				return this.$store.state.nickName;
			},
			//05.通过计算属性给cartCount赋值
			cartCount(){
				return this.$store.state.cartCount;
			},
		},
		//04.页面加载和刷新时，检查并修改登录状态
		mounted() {
			this.checkLogin();
		},
		methods:{
				...
				axios.post("/users/login",{
					...
					userPwd:this.userPwd
						...
						if(res.status=="0"){
							this.errorTip = false;
							this.loginModalFlag = false;
							//01_6.通过commit方法更新state数据
							this.$store.commit("updateUserInfo",res.result.userName);
							//this.nickName = res.result.userName;
							//02_2.用户登录成功后，查询一次购物车商品数量
							this.getCartCount();
			...
			logout(){
				axios.post("/users/logout").then((response)=>{//调用后端接口
					let res = response.data;//获取后端接口返回的cookie处理状态
					if(res.status=="0"){//如果cookie已清空
					//this.nickName = ''//取消页面login显示
					//02_3.取消页面login显示
